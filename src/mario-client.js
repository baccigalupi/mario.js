...
MarioClient.prototype.renderTree = function(compiled) {
  return new MarioClient.Tree(compiled).render(this.view, this.partials);
}

MarioClient.Tree = function(compiled) {
  this.tags = compiled.tags;
  this.texts = compiled.texts;
  this.content = this.texts.slice(0);
}

MarioClient.Tree.prototype.render = function renderTree(view, templates) {
  this.view = view;
...
MarioClient.Tag = function(complilation) {
  ...
  this.tree = complilation.tree && new MarioClient.Tree(complilation.tree);
};
